...
void	sha224_transform2_damnnorm(t_sha224_vars *v, u_int8_t *blk_cpy)
{
	int i;
...
void	sha224_transform3_damnnorm(t_sha224_vars *v, u_int32_t wb[])
{
	int i;
	...
	while (i < 64)
	{
		...
		(*v).temp1 = wb[7] + (*v).s1 + (*v).ch + g_sha224_k[i] + (*v).w[i];
...
void	sha224_transform4_damnnorm(t_sha224_ctx *ctx, u_int8_t hash[32])
{
	int i;
	int j;
...
void	sha224_transform(t_sha224_ctx *ctx, u_int8_t hash[32])
{
	...
	while (calc224_block(ctx->block, ctx))
	{
		i = -1;
		j = -1;
		blk_cpy = ctx->block;
		ft_memset(v.w, 0x00, sizeof(v.w));
		sha224_transform2_damnnorm(&v, blk_cpy);
		while (++i < 8)
			wb[i] = ctx->state[i];
		sha224_transform3_damnnorm(&v, wb);
		while (++j < 8)
			ctx->state[j] += wb[j];
	}
	sha224_transform4_damnnorm(ctx, hash);
}
